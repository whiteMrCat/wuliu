<template>
	<view class="release">
		<u-form :model="form" ref="uForm" label-width="200" label-align="center">
			...
			<u-form-item label="货物重量" prop="info">
				<u-input v-model="form.weight" placeholder="请输入货物重量/吨" />
			</u-form-item>
			<u-form-item label="货物类型" prop="info">
				<u-input v-model="form.goods_type" disabled @click="showType = true" placeholder="请选择货物类型" />
			</u-form-item>
			<u-form-item label="货物名称" prop="info">
				<u-input v-model="form.goods_name" placeholder="请输入货物名称" />
			</u-form-item>
...
<script>
	export default {
		...
		methods: {
			submitForm() {
				let _this = this
				this.$api.addInfoGoods({
					'startAddress': this.form.startAddress.address,
					'endAddress': this.form.endAddress.address,
